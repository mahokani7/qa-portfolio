...
from app.knowledge_base import get_chroma_collection, retrieve_context
...
"""
    ...
    def _build_no_rag_system_prompt(self) -> str:
        """RAG 없이(지식 검색을 사용하지 않고) LLM 자체 지식만으로 답변하게 하는 시스템 프롬프트.
        RAG on/off 비교(ablation)에서 'RAG를 끈' 대조군으로 사용한다."""
        return """
당신은 '교육과정 안내 챗봇'입니다. 참고할 수 있는 기준 정보(지식 베이스)가 제공되지 않았습니다.
당신이 이미 알고 있는 일반적인 지식만으로 사용자의 질문에 최선을 다해 답변하세요.
답변은 간결하고 명확하게 작성하세요.
"""

    def generate_response(self, user_question: str, use_rag: bool = True) -> str:
        """
        사용자의 질문을 받아 답변을 생성합니다.

        :param user_question: 사용자가 입력한 질문 문자열
        :param use_rag: True(기본)면 ChromaDB에서 기준 정보를 검색해 RAG로 답변한다.
                        False면 지식 검색 없이 LLM 자체 지식만으로 답변한다(RAG on/off 비교용 대조군).
        :return: 챗봇의 답변 문자열
        """
        try:
            if use_rag:
                ...
                system_prompt = self._build_no_rag_system_prompt()
