...
def list_chroma_indexed_files() -> List[str]:
    """
    업로드 폴더의 파일 목록이 아니라, 현재 ChromaDB 컬렉션에 실제로 임베딩되어 반영된 파일명만 반환합니다.
    (업로드만 하고 아직 '크로마 DB 재생성'을 누르지 않은 파일은 제외됩니다.)
    """
    ...
    sources = {m["source"] for m in stored.get("metadatas", []) if m and m.get("source")}
