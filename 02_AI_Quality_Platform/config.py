...
# Jira 결함관리 연동 (9단계, 선택 사항 — 미설정 시 jira_reporter가 자동으로 건너뜀)
JIRA_BASE_URL = os.getenv("JIRA_BASE_URL")        # 예: https://your-domain.atlassian.net
JIRA_EMAIL = os.getenv("JIRA_EMAIL")
JIRA_API_TOKEN = os.getenv("JIRA_API_TOKEN")
JIRA_PROJECT_KEY = os.getenv("JIRA_PROJECT_KEY")  # 예: AIQ
JIRA_ISSUE_TYPE = os.getenv("JIRA_ISSUE_TYPE", "Bug")  # 프로젝트에 있는 이슈 타입명 (예: Task, Bug, 작업)

# 4. 세부 디렉토리 경로 정의 (app/quality/tests/performance/monitoring/docs 구조 기준)
...
# 테스트 케이스 파일 경로 설정 (quality/ 로 이관됨)
...
def init_directories():
    ...
    directories = [DATA_DIR, QUALITY_DIR, REPORTS_DIR, DASHBOARD_DIR, KNOWLEDGE_DIR, KNOWLEDGE_UPLOAD_DIR, HISTORY_DIR]
